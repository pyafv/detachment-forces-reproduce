...
for P0_val in P0s:
    ...
    for v0 in v0s:
        if rank == 0:
            counter += 1
            print(f"\nRunning simulation for P0={P0_val:g}, v0={v0:g}: {counter}/{len(P0s)*len(v0s)}")
        ...
        va = v0                 # active velocity [30 microns/hr]
        ...
        if rank == 0:
            ...
            np.savez('data/P0_%g_v0_%g.npz' % (P0_val, v0), rupture_times=rupture_times_all, rupture_sizes=rupture_sizes_all)
            print('\nSaved data to data/P0_%g_v0_%g.npz' % (P0_val, v0))
