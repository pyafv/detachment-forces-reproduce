...
start, end = 0., 0.92
cmap = parula_map  # plt.cm.viridis or plasma
colors = cmap(np.linspace(start, end, len(dts)))

new_cmap = LinearSegmentedColormap.from_list('truncated_parula', cmap(np.linspace(start, end, 256)))

norm = LogNorm(vmin=min(dts), vmax=max(dts))
colors = new_cmap(norm(dts))
...
for dt in dts:
    ...
    kmf.fit(T, event_observed=E, label=r"$\Delta t=%g$" % dt, timeline=np.linspace(0, rupture_times.max()+dt, 1000))

    idx = np.where(np.array(dts) == dt)[0][0]
    kmf.plot_survival_function(ax=ax, color=colors[idx], clip_on=True)
...
cbar.ax.set_title(r'$\Delta t$', pad=8)
